...
//			indexSearcher = new IndexSearcher(LuceneUtil.getDirectory());
			...
			if("_time".equals(sortOp)) {
				...
				sort = new Sort(new SortField("score", SortField.STRING, true));
			}
			TopDocs topDocs;
			if(sort == null) {
...
//			int eIndex = dTitle.length() + 100;
			String dContent = doc.get("content");
//			if(dContent.length() > (eIndex + 250)) {
//				dContent = doc.get("content").substring(eIndex);
//			}
//			
			String hlContent = highlighter.getBestFragment(LuceneUtil.getAnalyzer(), "content", dContent);
			if(hlTitle!=null) {
